/*
 * plugin source from https://github.com/tilap/craco-sass-resources-loader
 */
...
module.exports = {
  overrideWebpackConfig: ({ webpackConfig, pluginOptions }) => {
    ...
    if (
      ...
      throw new Error(
        'craco-sass-resources-loader error: no valid webpackConfig.module.rules'
      );
    }
      ...
      if (Object.prototype.hasOwnProperty.call(rule, 'oneOf')) {
        rule.oneOf.forEach((oneOf, oneOfIndex) => {
          if (
            oneOf.test &&
            oneOf.use &&
            (`${oneOf.test}`.includes('scss') ||
              `${oneOf.test}`.includes('sass'))
          ) {
